...
const returnYoungerPeople = (array, age) => array.filter(person => person.age < age);

const youngerThan35 = returnYoungerPeople(peopleArray, 35);
...
const getTotalAge = array => array.reduce((total, person) => total + person.age, 0);

const totalAges = getTotalAge(peopleArray);
...
const addPointsToAll = (array, points) => array.forEach(user => user.points += points);
...
const addPointsOneUser = (username, points) => {
...
const filterAuthorAndPages = (authorName, pages) =>
  booksArray.filter(book => book.author === authorName && book.pages < pages);

const filteredBooks = filterAuthorAndPages('Harper Lee', 400);
...
const filteredCountries = filterCountries(countriesArray, ["United States", "Brazil", "Egypt", "Argentina"], 70000000);
